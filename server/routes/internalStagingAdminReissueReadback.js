...
} = require("../services/stagingPosAdminReissueReadbackService");
const {
  validateStandardSignedResponseFieldSet
} = require("../utils/posLicenceContract");
const { connectToDatabase } = require("../utils/db");

const STAGING_ADMIN_REISSUE_READBACK_PATH =
  "/internal/pos-licensing-staging-admin-reissue-readback";
...
function readConfiguredToken(env) {
  const token = env && env.POS_LICENSING_STAGING_ADMIN_REISSUE_READBACK_TOKEN;
  if (
    typeof token !== "string" ||
    token.length < 43 ||
    token.length > 128 ||
...
function shouldMountStagingAdminReissueReadbackEndpoint(env = process.env) {
  return clean(env.VERCEL) === "1" &&
    ...
    clean(env.POS_LICENSING_STAGING_ADMIN_REISSUE_READBACK_ENABLED).toLowerCase() === "true" &&
...
function projectSuccess(result, expectedKeyId) {
  if (
    ...
    !canonicalObjectId(result.installationRecordId) ||
    !canonicalObjectId(result.issueId) ||
  ...
  return {
    ...
    installationRecordId: canonicalObjectId(result.installationRecordId),
    issueId: canonicalObjectId(result.issueId),
...
function logSafeFailure(logger, reason) {
  ...
  try {
    if (logger && typeof logger.warn === "function") {
      logger.warn(`[staging-admin-reissue-readback] reason=${reason}`);
    }
  } catch {
...
function createStagingAdminReissueReadbackHandler(options = {}) {
  ...
  const runReadback = options.runReadback || readStagingAdminReissue;
  const logger = options.logger || console;

  return async function stagingAdminReissueReadbackHandler(req, res) {
    res.set("Cache-Control", "no-store");
    res.set("Pragma", "no-cache");
    if (!shouldMountStagingAdminReissueReadbackEndpoint(env)) {
    ...
    try {
      ...
      logSafeFailure(logger, "readback_failed");
      return res.status(503).json(UNAVAILABLE_RESPONSE);
    }
    const response = projectSuccess(result, config.keyId);
    if (!response) {
      logSafeFailure(logger, "invalid_readback_result");
      return res.status(503).json(UNAVAILABLE_RESPONSE);
    }
...
function mountStagingAdminReissueReadbackEndpoint(router, options = {}) {
  const env = options.env || process.env;
  if (!shouldMountStagingAdminReissueReadbackEndpoint(env)) {
    return false;
  }
  router.get(
    ...
    createStagingAdminReissueReadbackHandler({ ...options, env })
  );
  return true;
}
