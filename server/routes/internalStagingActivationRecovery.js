...
} = require("../services/stagingPosCommittedActivationRecoveryService");
const {
  validateStandardSignedResponseFieldSet
} = require("../utils/posLicenceContract");
const { connectToDatabase } = require("../utils/db");

const STAGING_ACTIVATION_RECOVERY_PATH =
  "/internal/pos-licensing-staging-activation-recovery";
...
const UNAVAILABLE_RESPONSE = Object.freeze({ message: "Activation recovery is unavailable." });
...
function readConfiguredToken(env) {
  const token = env && env.POS_LICENSING_STAGING_ACTIVATION_RECOVERY_TOKEN;
  if (
    typeof token !== "string" ||
    token.length < 43 ||
    token.length > 128 ||
...
function shouldMountStagingActivationRecoveryEndpoint(env = process.env) {
  return clean(env.VERCEL) === "1" &&
    ...
    clean(env.POS_LICENSING_STAGING_ACTIVATION_RECOVERY_ENABLED).toLowerCase() === "true" &&
...
function logSafeFailure(logger, reason) {
  ...
  try {
    if (logger && typeof logger.warn === "function") {
      logger.warn(`[staging-activation-recovery] reason=${reason}`);
    }
  } catch {
...
function createStagingActivationRecoveryHandler(options = {}) {
  ...
  const runRecovery = options.runRecovery || recoverStagingCommittedActivation;
  const logger = options.logger || console;
  const clock = options.clock || (() => new Date());

  return async function stagingActivationRecoveryHandler(req, res) {
    res.set("Cache-Control", "no-store");
    res.set("Pragma", "no-cache");
    if (!shouldMountStagingActivationRecoveryEndpoint(env)) {
...
function mountStagingActivationRecoveryEndpoint(router, options = {}) {
  const env = options.env || process.env;
  if (!shouldMountStagingActivationRecoveryEndpoint(env)) {
    return false;
  }
  router.post(
    ...
    createStagingActivationRecoveryHandler({ ...options, env })
  );
  return true;
}
